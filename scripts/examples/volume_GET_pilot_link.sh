...
do
    case $1 in
        -h|--help)
            printf "Get the Volume Information for Pilot Link Data Services\n"
            printf "\n"
            printf "Examples:\n"
            printf "  # Get the Volume Information for Pilot Link Data Services with the specified namespace\n"
            printf "  volume_GET_pilot_link.sh -n namespace\n"
            printf "\n"
            printf "  # Get the Volume Information for Pilot Link Data Services in the kubernetes cluster\n"
            printf "  volume_GET_pilot_link.sh\n"
            printf "\n"
            printf "Options:\n"
            printf "      -n, --namespace: The optional namespace get information for. If no namespace\n"
            printf "is provided all Pilot Link Data Services Volume Information will be shown\n"
            printf "      -v, --verbose: Display verbose output. If not provided only important fields will be\n"
            printf "displayed\n"
            printf "\n"
            printf "Usage:\n"
            printf "  volume_GET_pilot_link.sh [options]\n"
            printf "\n"
            exit 1
            ;;
        -n|--namespace)
...
do        
        ...
        if [ $IDX == 2 ]
        then
            printf "Volume GET for ${UDS_POD_ARRAY[0]}/${UDS_POD_ARRAY[1]}\n"
            ...
            then
                ...
                printf "[\n"
                echo "$OUTPUT" | grep -E "{|}|\"category\"|\"name\"|\"volumeId\"|\"uri\"" | grep -v "     \"name\""
                printf "]\n"
            fi
