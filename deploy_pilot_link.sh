...
do
    case $1 in
        -h|--help)
            ...
            printf "  deploy_pilot_link.sh -n nostore1 -t no -r http://api.lyve.seagate.com/accountuuid -u lpuser@email.com\n"
            printf "\n"
            printf "  # Deploy a Pilot Link deployment with the \"detect\" storage profile\n"
            printf "  deploy_pilot_link.sh -n detect1 -f pilot-link-config.json -t detect -r http://api.lyve.seagate.com/accountuuid -u lpuser@email.com\n"
            printf "\n"
            printf "  # Deploy a Pilot Link deployment with the \"detect\" storage profile to a specific kubernetes node\n"
            printf "  deploy_pilot_link.sh -n detect1 -f pilot-link-config.json -t detect -l pilotlinknodelabel -r http://api.lyve.seagate.com/accountuuid -u lpuser@email.com\n"
            printf "\n"
            printf "Options:\n"
